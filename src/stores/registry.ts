...
export const useRegistryStore = defineStore('registry', () => {
  ...
  const filteredRegistries = computed(() => {
    ...
    return registries.value.filter(r => r.name.toLowerCase().includes(q) || r.url.toLowerCase().includes(q))
  })
  ...
  async function fetchRegistries() {
    loading.value = true
    try {
      const [allRegs, current] = await Promise.all([api.getRegistries(), api.getCurrentRegistry()])
      registries.value = allRegs
      currentRegistry.value = current
    } catch (e) {
      ElMessage.error(t('registryStore.fetchFailed', { error: formatInvokeErrorMessage(t, e) }))
    } finally {
      loading.value = false
    }
  }
  ...
  async function switchRegistry(name: string) {
    try {
      ...
      currentRegistry.value = registries.value.find(r => r.name === name) || null
    } catch (e) {
      ElMessage.error(t('registryStore.switchFailed', { error: formatInvokeErrorMessage(t, e) }))
    }
  }
  ...
  async function deleteRegistry(name: string, options?: { silent?: boolean }) {
    ...
    try {
      ...
      currentRegistry.value = current
      const autoSwitched = prevCurrentName === name && current !== null && current.name !== name
      if (!silent) {
        if (autoSwitched) {
          ElMessage.success(
      ...
      ElMessage.error(t('registryStore.deleteFailed', { error: formatInvokeErrorMessage(t, e) }))
      throw e
    }
  }
  ...
  async function updateRegistry(name: string, newName: string, newUrl: string) {
    try {
